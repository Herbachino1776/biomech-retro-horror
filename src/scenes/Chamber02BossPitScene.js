...
export class Chamber02BossPitScene extends Phaser.Scene {
  constructor() {
  ...
  handleBossPitVictory() {
    if (this.hasProcessedBossPitVictory || this.majorEncounterResolution?.isResolutionActive('chamber02-bosspit')) {
      return;
    }
      ...
      stages: [
        {
          ...
          run: () => {
            ...
            if (!bossPitRunState.hasChamber02BossPitRewardGranted()) {
              const rewarded = grantMajorEncounterIntegrityReward(this.player, this.integrityRewardTracker, 'chamber02-bosspit-01-witness');
              if (rewarded) {
                bossPitRunState.markChamber02BossPitRewardGranted();
              }
            }
          }
        },
