...
const KILN_ENCOUNTER_POCKETS = [
  {
    id: 'kiln-condensers', label: 'CONDENSER THROAT', zoneX: 1280, zoneY: WORLD.floorY - 72, zoneWidth: 540, zoneHeight: 226,
    markerWidth: 328, markerHeight: 74, markerAlpha: 0.08, promptOffsetY: -138,
    enemies: [
      { type: 'basic01', x: 1160, y: PLAYER.startY, patrolDistance: 84 },
      { type: 'basic02', x: 1405, y: PLAYER.startY, patrolDistance: 110, wakeDelayMs: 140 }
    ]
  },
  {
    id: 'kiln-refinement', label: 'REFINEMENT KILN', zoneX: 2870, zoneY: WORLD.floorY - 74, zoneWidth: 760, zoneHeight: 236,
    markerWidth: 406, markerHeight: 82, markerAlpha: 0.1, promptOffsetY: -148,
    enemies: [
      { type: 'basic02', x: 2520, y: PLAYER.startY, patrolDistance: 120 },
      { type: 'elite', x: 2885, y: PLAYER.startY, patrolDistance: 82, wakeDelayMs: 180 },
      { type: 'basic01', x: 3200, y: PLAYER.startY, patrolDistance: 102, wakeDelayMs: 320 }
    ]
  },
  {
    id: 'kiln-threshold', label: 'BANISHER THRESHOLD', zoneX: 4680, zoneY: WORLD.floorY - 76, zoneWidth: 920, zoneHeight: 242,
    markerWidth: 472, markerHeight: 84, markerAlpha: 0.12, promptOffsetY: -154,
    enemies: [
      { type: 'basic01', x: 4340, y: PLAYER.startY, patrolDistance: 102 },
      { type: 'basic02', x: 4685, y: PLAYER.startY, patrolDistance: 116, wakeDelayMs: 160 },
      { type: 'elite', x: 5010, y: PLAYER.startY, patrolDistance: 78, wakeDelayMs: 320 }
    ]
  }
];
...
export class Sector02Chamber03Scene extends Phaser.Scene {
  ...
  createEncounterEnemy(enemyConfig, pocketConfig) {
    const baseConfig = enemyConfig.type === 'elite'
      ? KILN_SKITTER_ELITE
    ...
    this.applyGameplayReadabilitySupport(enemy.sprite, enemyConfig.type === 'elite'
    ...
    this.encounterPockets.forEach((pocket) => {
      ...
      if (playerInsidePocket && !pocket.activated) {
        ...
        pocket.enemies.forEach((enemy, index) => {
          if (!enemy.dead) {
            enemy.pocketWakeAtTime = time + (enemy.wakeDelayMs ?? index * 140);
          }
        });
      }

      pocket.enemies.forEach((enemy) => {
        if (!enemy.dead && pocket.activated && !enemy.awakened && time >= (enemy.pocketWakeAtTime ?? 0)) {
          enemy.awakened = true;
        }
      });
