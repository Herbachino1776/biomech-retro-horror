...
const CHAMBER = {
  sceneKey: 'Sector03Chamber02BossChamberScene',
  worldWidth: 1960,
  spawnX: 344,
...
const MISASSIGNED_SERAPH_BOSS = {
  ...
  windupDriftSpeed: 10,
  attackSpeed: 208,
  attackLiftVelocity: -128,
  hitPulseMs: 280,
  hurtRecoverMs: 230,
  hurtRecoilVelocityX: 104,
  hurtRecoilVelocityY: -66,
  body: { width: 98, height: 128, offsetX: 92, offsetY: 144 },
...
const VICTORY = {
  preExplosionShakeMs: 3200,
  preExplosionShakeIntensity: 0.007,
  goreFountainCadenceMs: 84,
  explosionFadeStartDelayMs: 110,
  explosionFadeDurationMs: 360,
  postExplosionDespawnDelayMs: 620
};
...
export class Sector03Chamber02BossChamberScene extends Phaser.Scene {
  constructor() {
  ...
  createBackdrop() {
    ...
    if (this.textures.exists(ASSET_KEYS.sector03Chamber02BackgroundClimaxSanctum)) {
      this.add.image(CHAMBER.worldWidth / 2, 210, ASSET_KEYS.sector03Chamber02BackgroundClimaxSanctum)
  ...
  handleBossVictory() {
    const encounterId = 'sector03-chamber02-major-boss';
  ...
  startVictoryGoreFountain() {
    ...
    const spawnFountainBurst = () => {
      ...
      triggerSector02BlackOilBlowout(this, {
        ...
        y: (this.boss.sprite.body?.bottom ?? this.boss.sprite.y) - Phaser.Math.Between(106, 164),
        depth: this.boss.sprite.depth + 0.38,
        scale: Phaser.Math.FloatBetween(0.8, 1.02),
        durationMs: 580,
        burstCount: 62,
        sprayCount: 84,
        mistCount: 9,
  ...
  tryUseExitAltar(mobileInput) {
    ...
    this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.scene.start('Sector03Chamber03Scene', {
        fromScene: this.scene.key,
        enteredFrom: this.scene.key
      });
    });
  ...
  setupMobileUiCamera() {
    ...
    this.uiCamera = this.cameras.add(0, 0, this.scale.width, this.scale.height, false, 'Sector03Chamber02BossChamberMobileUiCamera');
